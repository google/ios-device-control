...
final class SimulatorDeviceImpl implements SimulatorDevice {
  ...
  public IosDeviceSocket openWebInspectorSocket() throws IosDeviceException {
    try {
      Optional<IosDevice> device = SimulatorDeviceHost.INSTANCE.deviceOnInspectorPort();
      if (device.isPresent() && device.get().equals(this)) {
        return IosDeviceSocket.wrap(this, new Socket("::1", 27753));
      } else {
        throw new IosDeviceException(this, "simulator is not on the inspector port");
      }
    } catch (IOException e) {
      throw new IosDeviceException(this, e);
    }
